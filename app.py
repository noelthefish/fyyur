...
class Venue(db.Model):
    ...
    genres = db.Column(db.ARRAY(db.String(120)))                 # The Genres the Venus prefers
    website = db.Column(db.String())
    seeking_talent = db.Column(db.Boolean, nullable=False) # True/False only
    seeking_description = db.Column(db.String())           # Some waffle they can enter
...
class Artist(db.Model):
    ...
    state = db.Column(db.String(120))
    phone = db.Column(db.String(120))
    genres = db.Column(db.ARRAY(db.String(120))) # The genres they classify themselves as
    image_link = db.Column(db.String(500))
    facebook_link = db.Column(db.String(120))

# TODO: implement any missing fields, as a database migration using Flask-Migrate
    website = db.Column(db.String())
    seeking_venue = db.Column(db.Boolean, nullable=False)   # True/False only
    seeking_description = db.Column(db.String())            # Somw pretentious band description
...
class Show(db.Model):
    __tablename__ = 'Show'
    id = db.Column(db.Integer, primary_key=True)
    start_time = db.Column(db.DateTime)                                         # Show date and time
    venue_id = db.Column(db.Integer, db.ForeignKey('Venue.id'), nullable=False) # Venue Id used for Foreign Key
    venue_name = db.relationship('Venue', backref=db.backref('shows'))
    artist_id = db.Column(db.Integer, db.ForeignKey('Artist.id'), nullable=False) # Artist Id used for foreign key
...
def search_venues():
  ...
  venues = db.session.query(Venue).filter(Venue.name.ilike('%' + search_term + '%')).all() # ilike case insensitive
  data = []
...
# This has been moved from below back into the venue section for clarity
...
def edit_artist(artist_id):
  ...
  form.process()
  return render_template('forms/edit_artist.html', form=form, artist=artist) # TODO: populate form with fields from artist with ID <artist_id>
